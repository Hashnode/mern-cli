...
var debug               = require('debug')('mern-cli:cli');
var debugTesting        = require('debug')('mern-cli:testing');
...
CLI.prototype.run = function(environment) {
  return Promise.hash(environment).then(function(environment) {
    ...
    getOptionArgs('--verbose', commandArgs).forEach(function(arg){
      process.env['MERN_VERBOSE_' + arg.toUpperCase()] = 'true';
    });
    ...
    if (!platform.isValid && !this.testing) {
      if (platform.isDeprecated) {
        this.ui.writeDeprecateLine('Node ' + process.version +
                                   ' is no longer supported by Mern CLI. Please update to a more recent version of Node');
      }

      if (platform.isUntested) {
        this.ui.writeWarnLine('WARNING: Node ' + process.version +
                              ' has currently not been tested against Mern CLI and may result in unexpected behaviour.');
      }
    }

    var skipVersionBanner = commandArgs.indexOf('--json') !== -1;
    if (!skipVersionBanner) {
      this.ui.writeLine('version: ' + mernCLIVersion());
    }
    ...
    if(!this.testing) {
      ...
      if (environment.project.isMernCLIProject() && !skipInstallationCheck) {
