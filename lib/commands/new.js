...
module.exports = Command.extend({
  name: 'new',
  description: 'Creates a new directory and runs ' + chalk.green('mern init') + ' in it.',
  ...
  run: function(commandOptions, rawArgs) {
    ...
    if (!packageName) {
      message = chalk.yellow('The `mern ' + this.name + '` command requires a ' +
                             'name to be specified. For more details, use `mern help`.');
    ...
    if (packageName === '.') {
      message = 'Trying to generate an application structure in this directory? Use `mern init` instead.';
