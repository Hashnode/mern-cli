...
var debug               = require('debug')('mern-cli:command');
...
function Command() {
  ...
  this.isWithinProject = this.project.isMernCLIProject();
...
Command.prototype.validateAndRun = function(args) {
  ...
  if (this.works === 'insideProject' && !this.isWithinProject) {
    return Promise.reject(new SilentError(
      'You have to be inside an mern-cli project in order to use ' +
  ...
  if (this.works === 'outsideProject' && this.isWithinProject) {
    return Promise.reject(new SilentError(
      ...
      ' command inside an mern-cli project.'
    ));
  }
  ...
  return this.run(commandOptions, commandOptions.args);
};
  ...
  if (isValidAlias(alias, option.type)) {
    ...
    if (Array.isArray(alias)) {
      ...
      try {
        ...
        var debug = require('debug')('mern-cli/models/command');
        debug(e);
      }
    }
  ...
  var validateParsed = function(key) {
    ...
    if (!commandOptions.hasOwnProperty(key) && key !== 'argv' && key !== 'h' && key !== 'help') {
      this.ui.writeLine(chalk.yellow('The option \'--' + key + '\' is not registered with the ' + this.name + ' command. ' +
                        'Run `mern ' + this.name + ' --help` for a list of supported options.'));
    }
...
/*
  ...
  Basic help looks like this:
      ...
      mern generate <blueprint> <options...>
